...
public class PartimerInfoViewActivity extends AppCompatActivity {
    ...
    public void onResume() {
        ...
        handler.postDelayed(new Runnable() {
            @Override
            public void run() {
                Picasso.with(PartimerInfoViewActivity.this).load(Info.p_avatar).into(iv_partimer_avatar);
            }
        }, 800);
    }
    private void initView(){
        ...
        e.update(eid,new UpdateListener() {
            ...
            public void done(BmobException e) {
                if (e == null) {
                    Toast.makeText(PartimerInfoViewActivity.this, "录用成功", Toast.LENGTH_SHORT).show();
                } else {
